...
class LinearClassifier(object):
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        margins[np.arange(num_train), y_batch] = 0  # Do not consider correct class in loss

        # Compute the loss
        loss = np.sum(margins) / num_train
        loss += reg * np.sum(self.W * self.W)  # Regularization term

        # Compute the gradient
        margins[margins > 0] = 1
        valid_margin_count = margins.sum(axis=1)
        margins[np.arange(num_train), y_batch] -= valid_margin_count
        dW = X_batch.T.dot(margins) / num_train
